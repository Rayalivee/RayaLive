...
import { get, ref as fbRef } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-database.js";
...
const els = {
  ...
  mhSubtitle: document.getElementById("mh-subtitle"),
  mhList: document.getElementById("mh-list"),
  mhClose: document.getElementById("mh-close"),
  ...
  tScheduleStart: document.getElementById("t-schedule-start"),
  tInviteOnly: document.getElementById("t-invite-only"),
  tInviteWrap: document.getElementById("t-invite-wrap"),
  tInviteFilter: document.getElementById("t-invite-filter"),
  tInviteList: document.getElementById("t-invite-list"),
  tInviteCount: document.getElementById("t-invite-count"),
  tLimitGamesCheck: document.getElementById("t-limit-games-check"),
  tMaxGames: document.getElementById("t-max-games"),
  tMaxGamesLabel: document.getElementById("t-max-games-label"),
  ...
  tournamentNextModal: document.getElementById("tournament-next-modal"),
  tournamentNextText: document.getElementById("tournament-next-text"),
  tournamentNextCountdown: document.getElementById("tournament-next-countdown"),
  tournamentNextYes: document.getElementById("tournament-next-yes"),
  tournamentNextNo: document.getElementById("tournament-next-no"),
...
let invitePickerPlayers = [];
let inviteSelected = {};

// ——— Tema claro / oscuro ———

function setServiceStatus(key, state, label) {
  const el = document.querySelector('[data-status="' + key + '"]');
  if (!el) return;
  el.classList.remove("ok", "warn", "down", "check");
...
async function refreshServiceStatuses() {
  const updated = document.getElementById("settings-status-updated");
  if (updated) updated.textContent = "Comprobando servicios…";

  const onlineBrowser = typeof navigator === "undefined" || navigator.onLine !== false;

  async function probe(path) {
    const t0 = Date.now();
    await get(fbRef(db, path));
    return Date.now() - t0;
  }

  let firebaseOk = false;
  try {
    const connSnap = await get(fbRef(db, ".info/connected"));
    firebaseOk = connSnap.exists() && connSnap.val() === true;
  } catch (e) {
    firebaseOk = false;
  }

  if (!firebaseOk && onlineBrowser) {
    try {
      await probe("tournaments");
      firebaseOk = true;
    } catch (e) {
      try {
        await probe("news");
        ...
        firebaseOk = onlineBrowser;
      }
    }
  }

  setServiceStatus("online", firebaseOk ? "ok" : "down", firebaseOk ? "Activo" : "Caído");

  async function checkService(key, path) {
    if (!firebaseOk && !onlineBrowser) {
      setServiceStatus(key, "down", "Caído");
      return;
    }
    try {
      const ms = await probe(path);
      setServiceStatus(key, ms > 3000 ? "warn" : "ok", ms > 3000 ? "Actualizando" : "Activo");
    } catch (e) {
      setServiceStatus(key, onlineBrowser ? "ok" : "down", onlineBrowser ? "Activo" : "Caído");
    }
  }

  await checkService("tournaments", "tournaments");
  await checkService("matchmaking", "matchmaking");
  await checkService("news", "news");
  if (userId) await checkService("friends", "friends/" + userId);
  else setServiceStatus("friends", "warn", "Sin sesión");
...
function applyTheme(dark) {
  ...
  try {
    localStorage.setItem("rayalive_theme", dark ? "dark" : "light");
  } catch (e) {}
  if (els.themeToggle) els.themeToggle.checked = !!dark;
  if (els.themeHint) {
    els.themeHint.textContent = dark ? "Tema oscuro activo" : "Tema claro activo";
  }
}
function initTheme() {
  let dark = false;
  try {
    dark = localStorage.getItem("rayalive_theme") === "dark";
  } catch (e) {}
  applyTheme(dark);
}
initTheme();
...
let tournamentNextTimer = null;
let tournamentNextSeconds = 0;
let tournamentStartTimers = {};
let tournamentEndTimers = {};
...
if (els.btnSettings) {
  els.btnSettings.addEventListener("click", () => {
    if (els.settingsVersion) {
      const v = document.getElementById("welcome-version-num");
      els.settingsVersion.textContent = (v && v.textContent) || "3.0";
    }
    if (els.themeToggle) {
...
if (els.settingsBack) {
  els.settingsBack.addEventListener("click", () => showView("menu"));
}
if (els.themeToggle) {
...
if (els.mhClose) {
  els.mhClose.addEventListener("click", () => {
    if (els.matchHistoryModal) els.matchHistoryModal.classList.add("hidden");
  });
}
...
async function startTournamentPlay(tournamentId) {
  ...
  const playGate = canPlayInTournament(t);
  if (!playGate.ok) {
    showToast(playGate.reason);
    activeTournamentId = null;
    return;
  }
  const gamesGate = canPlayMoreGames(t, userId);
  if (!gamesGate.ok) {
    showToast(gamesGate.reason);
    activeTournamentId = null;
    return;
  }
...
/** Reloj cada segundo: inicio y fin a la hora exacta */
let tournamentClockStarted = false;
const tournamentToastShown = { start: {}, end: {} };

function ensureTournamentClock() {
  if (tournamentClockStarted) return;
  tournamentClockStarted = true;
  setInterval(() => {
    tickTournamentSchedule();
  }, 1000);
}

function tickTournamentSchedule() {
  if (!tournamentList || !tournamentList.length) return;
  const now = Date.now();
  tournamentList.forEach((t) => {
    if (!t || !t.id || !t.title) return;

    // INICIO exacto
    const schedule = t.scheduleStart !== false;
    ...
    if (
      ...
      maybeAutoStartTournament(t, username)
        .then((ok) => {
          if (ok && !tournamentToastShown.start[t.id]) {
            tournamentToastShown.start[t.id] = true;
            showToast("El torneo «" + t.title + "» ha comenzado.");
          }
        })
        .catch(() => {});
    }
    ...
    if (
      ...
      maybeAutoFinishTournament(t, username)
        .then((ok) => {
          if (ok !== false && !tournamentToastShown.end[t.id]) {
            tournamentToastShown.end[t.id] = true;
            showToast("El torneo «" + t.title + "» ha finalizado.");
            if (activeTournamentId === t.id || window.__lastRoomTournamentId === t.id) {
              // Si no estás en mitad de partida jugando, sacar
              const inLiveGame =
                currentRoomCode &&
                views.game &&
                !views.game.classList.contains("hidden");
              if (!inLiveGame) {
                forceExitIfTournamentOver(t.id, "El torneo ha finalizado.");
              }
            }
          }
        })
...
function scheduleTournamentTimers(t) {
  ensureTournamentClock();
  // Comprobación inmediata también
  if (t) {
    const now = Date.now();
    if (
      t.scheduleStart !== false &&
      t.startAt &&
      now >= Number(t.startAt) &&
      (t.status === "upcoming" || t.status === "open")
    ) {
      maybeAutoStartTournament(t, username).catch(() => {});
    }
    if (
      Number(t.endAt) > 1577836800000 &&
      now >= Number(t.endAt) &&
      t.status !== "finished" &&
      t.status !== "cancelled"
    ) {
...
function renderTournamentsList() {
  ...
  // No mostrar torneos sin título
  tournamentList = (tournamentList || []).filter((t) => t && t.title && String(t.title).trim());
...
function clearTournamentNextTimer() {
  if (tournamentNextTimer) {
    clearInterval(tournamentNextTimer);
    tournamentNextTimer = null;
  }
}

function hideTournamentNextModal() {
  clearTournamentNextTimer();
  if (els.tournamentNextModal) els.tournamentNextModal.classList.add("hidden");
}

function exitTournamentMatchToMenu() {
  hideTournamentNextModal();
  clearTournamentNextTimer();
  try {
    closeResultModal();
  } catch (e) {}
  if (els.resultLeaveBtn) els.resultLeaveBtn.classList.remove("hidden");
  if (els.rematchAccept) els.rematchAccept.textContent = "Aceptar";
  if (els.rematchReject) els.rematchReject.textContent = "Rechazar";
  cleanupRoomWatch();
  currentRoomCode = null;
  window.__lastRoomTournamentId = null;
  activeTournamentId = null;
  window.__pendingTournamentNext = null;
  showView("menu");
}

/** Si el torneo acaba mientras buscas o tras partida, saca al menú con mensaje claro */
async function forceExitIfTournamentOver(tournamentId, reason) {
  if (!tournamentId) return false;
  try {
    const t = await getTournament(tournamentId);
    if (!t || isTournamentClosed(t) || t.status === "finished" || t.status === "cancelled") {
      try { await cancelTournamentQueue(tournamentId, userId); } catch (e) {}
      cleanupRoomWatch();
      currentRoomCode = null;
      activeTournamentId = null;
      window.__lastRoomTournamentId = null;
      ...
      showToast(reason || "El torneo ha finalizado.");
      showView("menu");
      return true;
    }
  } catch (e) {}
...
async function openPlayerMatchHistory(tournamentId, playerId, playerName) {
  if (!els.matchHistoryModal) return;
  if (els.mhTitle) els.mhTitle.textContent = "Partidas de " + playerName;
  if (els.mhSubtitle) els.mhSubtitle.textContent = "Cargando…";
  if (els.mhList) els.mhList.innerHTML = "";
  els.matchHistoryModal.classList.remove("hidden");
  try {
    const list = await getPlayerTournamentHistory(tournamentId, playerId);
    if (els.mhSubtitle) {
      els.mhSubtitle.textContent =
        list.length === 0
          ? "Aún no hay partidas guardadas."
          : list.length + " partida" + (list.length === 1 ? "" : "s");
    }
    if (!els.mhList) return;
    els.mhList.innerHTML = "";
    list.forEach((m) => {
      const item = document.createElement("div");
      item.className = "mh-item " + (m.iWon ? "mh-win" : "mh-loss");
      const blueNames = (m.blue || []).map((p) => p.name).join(", ") || "—";
      const redNames = (m.red || []).map((p) => p.name).join(", ") || "—";
      const vs = document.createElement("div");
      vs.className = "mh-vs";
      const left = document.createElement("span");
      left.textContent = blueNames + " vs " + redNames;
      const tag = document.createElement("span");
      tag.className = "mh-winner-tag";
      tag.textContent = m.iWon ? "Victoria" : "Derrota";
      vs.appendChild(left);
      vs.appendChild(tag);
      item.appendChild(vs);
      const size = Number(m.boardSize) || 3;
      const board = document.createElement("div");
      board.className = "mh-mini-board";
      board.style.gridTemplateColumns = "repeat(" + size + ", 1fr)";
      const cells = Array.isArray(m.board) ? m.board : [];
      for (let i = 0; i < size * size; i++) {
        const cell = document.createElement("span");
        const v = cells[i] || "";
        cell.textContent = v;
        if (v === "X") cell.className = "x";
        if (v === "O") cell.className = "o";
        board.appendChild(cell);
      }
      item.appendChild(board);
      els.mhList.appendChild(item);
    });
  } catch (e) {
    console.warn(e);
    if (els.mhSubtitle) els.mhSubtitle.textContent = "No se pudo cargar el historial.";
  }
}


function showTournamentNextInResultModal(tournamentId, outcome) {
  clearTournamentNextTimer();
  hideTournamentNextModal();
  ...
  // Reutilizar cajas de rematch para los botones de torneo
  if (els.rematchAskBox) els.rematchAskBox.classList.add("hidden");
  if (els.rematchRequestBox) els.rematchRequestBox.classList.remove("hidden");

  const reqText = document.getElementById("rematch-request-text");
  if (reqText) {
    reqText.textContent =
      outcome === "win"
        ? "¿Buscar nueva partida de torneo?"
        : "¿Buscar nueva partida de torneo?";
  }

  // Countdown bajo el texto
  let cdEl = document.getElementById("tournament-result-countdown");
  if (!cdEl && els.rematchRequestBox) {
    cdEl = document.createElement("p");
    cdEl.id = "tournament-result-countdown";
    cdEl.className = "status-line";
    els.rematchRequestBox.insertBefore(cdEl, els.rematchRequestBox.querySelector(".rematch-actions"));
  }

  tournamentNextSeconds = 3;
  if (cdEl) cdEl.textContent = tournamentNextSeconds + "…";
  ...
  const onYes = () => {
    ...
    startTournamentPlay(tournamentId);
  };
  const onNo = () => {
    clearTournamentNextTimer();
    exitTournamentMatchToMenu();
  };

  if (els.rematchAccept) {
    els.rematchAccept.textContent = "Sí";
    els.rematchAccept.onclick = onYes;
  }
  if (els.rematchReject) {
    els.rematchReject.textContent = "Salir al menú";
    els.rematchReject.onclick = onNo;
  }
  if (els.resultLeaveBtn) {
    els.resultLeaveBtn.classList.add("hidden");
  }

  tournamentNextTimer = setInterval(() => {
    tournamentNextSeconds -= 1;
    if (cdEl) {
      cdEl.textContent =
        tournamentNextSeconds > 0 ? tournamentNextSeconds + "…" : "0…";
    }
    if (tournamentNextSeconds <= 0) {
      clearTournamentNextTimer();
      exitTournamentMatchToMenu();
    }
  }, 1000);
...
function showTournamentNextPrompt(tournamentId, outcome) {
  if (!els.tournamentNextModal) {
    exitTournamentMatchToMenu();
    return;
  }
  clearTournamentNextTimer();
  ...
  const msg =
    outcome === "win"
      ? "¡Has ganado! ¿Buscar nueva partida?"
      : "Partida terminada. ¿Buscar nueva partida?";
  if (els.tournamentNextText) els.tournamentNextText.textContent = msg;

  tournamentNextSeconds = 3;
  if (els.tournamentNextCountdown) {
    els.tournamentNextCountdown.textContent = tournamentNextSeconds + "…";
  }
  els.tournamentNextModal.classList.remove("hidden");
  ...
  if (els.tournamentNextYes) {
    els.tournamentNextYes.onclick = onYes;
  }
  if (els.tournamentNextNo) {
    els.tournamentNextNo.onclick = onNo;
  }

  tournamentNextTimer = setInterval(() => {
    tournamentNextSeconds -= 1;
    if (els.tournamentNextCountdown) {
      els.tournamentNextCountdown.textContent =
        tournamentNextSeconds > 0 ? tournamentNextSeconds + "…" : "0…";
    }
    if (tournamentNextSeconds <= 0) {
      clearTournamentNextTimer();
...
function renderTournamentDetail(t) {
  ...
  if (els.tdReqs) {
    ...
    if (t.maxGames > 0) parts.push("Máx. " + t.maxGames + " partidas");
    if (t.inviteOnly) {
      const inv = Object.keys(t.invited || {}).length;
      parts.push("Solo invitados (" + inv + ")");
    }
    if (t.scheduleStart !== false && t.startAt && Date.now() < t.startAt) {
      parts.push("Solo inscripciones hasta el inicio");
    }
  ...
  if (els.tdActions) {
    ...
    if (closed) {
      ...
      join.addEventListener("click", async () => {
        try {
          ...
          if (els.tdError) els.tdError.textContent = friendlyError(e, "No se pudo inscribir. Revisa requisitos o si el torneo ha terminado.");
        }
      });
      els.tdActions.appendChild(join);
      if (t.status === "live" && check.ok) {
        const liveHint = document.createElement("p");
        liveHint.className = "hint";
        liveHint.textContent = "Puedes inscribirte aunque el torneo ya esté en curso.";
        els.tdActions.appendChild(liveHint);
      }
      if (!check.ok) {
    ...
    // Jugar solo si inscrito y se puede jugar
    if (!closed && joined) {
      const playOk = canPlayInTournament(t);
      const gamesOk = canPlayMoreGames(t, userId);
      if (playOk.ok && gamesOk.ok) {
        ...
        const why = document.createElement("p");
        why.className = "hint";
        why.textContent = (!playOk.ok ? playOk.reason : gamesOk.reason) || "";
        els.tdActions.appendChild(why);
      }
      if (t.maxGames > 0) {
        const lim = document.createElement("p");
        lim.className = "hint";
        const played = (t.players && t.players[userId] && t.players[userId].gamesPlayed) || 0;
        lim.textContent = "Partidas: " + played + " / " + t.maxGames;
        els.tdActions.appendChild(lim);
      }
    }
  }
  ...
  if (els.tdLeaderboard) {
    ...
    if (!lb.length) {
      ...
      lb.forEach((p, i) => {
        ...
        const gamesN = p.gamesPlayed || 0;
        score.innerHTML = "";
        score.appendChild(
          document.createTextNode(p.points + " pts · " + p.wins + "V · ")
        );
        const gamesBtn = document.createElement("button");
        gamesBtn.type = "button";
        gamesBtn.className = "ranking-games-btn";
        gamesBtn.textContent =
          gamesN +
          " partida" +
          (gamesN === 1 ? "" : "s") +
          (t.maxGames > 0 ? "/" + t.maxGames : "");
        gamesBtn.title = "Ver tableros de " + (p.name || "jugador");
        gamesBtn.addEventListener("click", (ev) => {
          ev.stopPropagation();
          openPlayerMatchHistory(t.id, p.id, p.name || "Jugador");
        });
        score.appendChild(gamesBtn);
        li.appendChild(pos);
        li.appendChild(name);
...
if (els.btnTournaments) {
  els.btnTournaments.addEventListener("click", () => {
    ...
    if (els.tournamentAdmin) {
      if (canManageTournaments(username)) {
        els.tournamentAdmin.classList.remove("hidden");
        if (els.tInviteOnly) els.tInviteOnly.checked = false;
        if (typeof setInviteUIVisible === "function") setInviteUIVisible(false);
      } else {
        els.tournamentAdmin.classList.add("hidden");
      }
    }
...
async function loadInvitePickerPlayers() {
  if (!els.tInviteList) return;
  els.tInviteList.innerHTML = "<p class='hint'>Cargando todos los jugadores…</p>";
  try {
    invitePickerPlayers = await getAllPlayers();
    renderInvitePickerList();
  } catch (e) {
    console.error(e);
    els.tInviteList.innerHTML = "<p class='error-text'>No se pudieron cargar los jugadores.</p>";
  }
}

function renderInvitePickerList() {
  if (!els.tInviteList) return;
  const q = ((els.tInviteFilter && els.tInviteFilter.value) || "").trim().toLowerCase();
  els.tInviteList.innerHTML = "";
  let shown = 0;
  invitePickerPlayers.forEach((p) => {
    if (!p || !p.id) return;
    const name = p.name || "Sin nombre";
    const idStr = String(p.id);
    if (q && !name.toLowerCase().includes(q) && !idStr.toLowerCase().includes(q)) {
      return;
    }
    shown++;
    const row = document.createElement("label");
    row.className = "invite-player-row";
    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.checked = !!inviteSelected[p.id];
    cb.addEventListener("change", () => {
      if (cb.checked) {
        inviteSelected[p.id] = { name: name };
      } else {
        delete inviteSelected[p.id];
      }
      updateInviteCount();
    });
    ...
    els.tInviteList.appendChild(row);
  });
  if (!shown) {
    els.tInviteList.innerHTML = invitePickerPlayers.length
      ? "<p class='hint'>Ningún jugador coincide con la búsqueda.</p>"
      : "<p class='hint'>No hay jugadores en la base de datos.</p>";
  }
  updateInviteCount();
}

function updateInviteCount() {
  if (!els.tInviteCount) return;
  const n = Object.keys(inviteSelected).length;
  els.tInviteCount.textContent =
    n + " seleccionado" + (n === 1 ? "" : "s") +
    (invitePickerPlayers.length ? " · " + invitePickerPlayers.length + " en total" : "");
}

function setInviteUIVisible(on) {
  if (els.tInviteWrap) {
    if (on) els.tInviteWrap.classList.remove("hidden");
    else {
      els.tInviteWrap.classList.add("hidden");
      if (els.tInviteList) els.tInviteList.innerHTML = "";
      if (els.tInviteFilter) els.tInviteFilter.value = "";
      if (els.tInviteCount) els.tInviteCount.textContent = "";
      inviteSelected = {};
      invitePickerPlayers = [];
    }
  }
...
if (els.tInviteOnly) {
  // Al cargar: siempre oculto si no está marcado
  setInviteUIVisible(!!els.tInviteOnly.checked);
  els.tInviteOnly.addEventListener("change", async () => {
    const on = els.tInviteOnly.checked;
    setInviteUIVisible(on);
    if (on) {
      inviteSelected = {};
      await loadInvitePickerPlayers();
    }
  });
} else if (els.tInviteWrap) {
  els.tInviteWrap.classList.add("hidden");
}
if (els.tInviteFilter) {
  els.tInviteFilter.addEventListener("input", () => renderInvitePickerList());
}

if (els.tLimitGamesCheck) {
  const maxWrap = document.getElementById("t-max-games-wrap");
  els.tLimitGamesCheck.addEventListener("change", () => {
    const on = els.tLimitGamesCheck.checked;
    if (maxWrap) maxWrap.classList.toggle("hidden", !on);
  });
}
...
if (els.tCreate) {
  els.tCreate.addEventListener("click", async () => {
    ...
    try {
      ...
      if (els.tInviteOnly && els.tInviteOnly.checked) {
        const n = Object.keys(inviteSelected).length;
        if (n === 0) {
          throw new Error("Selecciona al menos un jugador invitado, o desactiva la lista de invitados.");
        }
      }
      await createTournament(
        {
          ...
          scheduleStart: els.tScheduleStart ? els.tScheduleStart.checked : true,
          limitGames: els.tLimitGamesCheck ? els.tLimitGamesCheck.checked : false,
          maxGames: els.tMaxGames ? els.tMaxGames.value : 20,
          inviteOnly: els.tInviteOnly ? els.tInviteOnly.checked : false,
          invited: els.tInviteOnly && els.tInviteOnly.checked ? { ...inviteSelected } : {},
...
function openResultModalOnline(
  ...
  if (!room.tournamentId) {
    ...
    if (els.rematchAccept) els.rematchAccept.textContent = "Aceptar";
    if (els.rematchReject) els.rematchReject.textContent = "Rechazar";
    const cdEl = document.getElementById("tournament-result-countdown");
    if (cdEl) cdEl.remove();
  }
  ...
  // En torneo: no revancha → Sí buscar / Salir (solo torneos)
  if (room.tournamentId) {
    if (els.rematchAskBox) els.rematchAskBox.classList.add("hidden");
    if (els.rematchRequestBox) els.rematchRequestBox.classList.add("hidden");
    if (els.rematchAskBtn) els.rematchAskBtn.classList.add("hidden");
    if (els.rematchWaitingText) els.rematchWaitingText.classList.add("hidden");

    // Montar prompt dentro del modal de resultado
    const tid = room.tournamentId;
    const pending = window.__pendingTournamentNext;
    const outcome = (pending && pending.outcome) || (room.winner === mySymbol ? "win" : "loss");

    getTournament(tid)
      .then((tInfo) => {
        const stillPlayable =
          tInfo &&
          !isTournamentClosed(tInfo) &&
          (tInfo.status === "live" || tInfo.status === "open");

        if (!stillPlayable) {
          // Torneo acabado → salir al menú tras un momento
          setTimeout(() => {
            try { closeResultModal(); } catch (e) {}
            hideTournamentNextModal();
            cleanupRoomWatch();
            ...
            activeTournamentId = null;
            window.__pendingTournamentNext = null;
            showToast("El torneo ha finalizado. Partida contada.");
            showView("menu");
          }, 1500);
          return;
        }

        // Torneo activo: mostrar Sí / Salir con cuenta atrás en el modal de resultado
        showTournamentNextInResultModal(tid, outcome);
      })
      .catch(() => {
        // Si falla la lectura, igual ofrecer buscar
        showTournamentNextInResultModal(tid, outcome);
      });
  } else if (!requestedBy) {
...
function renderOnlineRoom(
  ...
  if (!finished && currentPlayer) {
    ...
    if (room.tournamentId && room.suddenDeath) {
      line += " · Muerte súbita (sin empates)";
    }
  ...
  if (finished) {
    ...
    if (
      ...
      if (room.tournamentId) {
        // Torneo: solo clasifica del torneo. NO monedas, NO racha.
        if (outcome !== "draw") {
          recordMyTournamentResult(room.tournamentId, userId, outcome).catch((e) =>
            console.warn("tournament result", e)
          );
          // Guardar tablero en historial (una sola vez por partida)
          if (window.__savedHistoryFor !== code) {
            window.__savedHistoryFor = code;
            saveTournamentMatchHistory(room.tournamentId, room, code).catch((e) =>
              console.warn("history save", e)
            );
          }
        }
        window.__pendingTournamentNext = {
          tid: room.tournamentId,
          outcome
        };
      } else {
        window.__pendingTournamentNext = null;
